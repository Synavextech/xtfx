...
export default function Admindashboard({ toggleTheme, theme }: AdmindashboardProps = {}) {
  ...
  return (
    <div className="min-h-screen bg-light-bg dark:bg-dark-bg text-light-primary dark:text-dark-primary flex flex-col p-6 transition-colors duration-200">
      ...
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-light-border dark:border-dark-border">
        <div>
          ...
          <p className="text-xs text-light-secondary dark:text-dark-secondary">Manage deposits, withdrawals, brokers, and analysis reports</p>
        </div>
        <div className="flex items-center gap-3">
          {toggleTheme && (
            <button
              onClick={toggleTheme}
              className="p-2 bg-light-panel dark:bg-dark-panel border border-light-border dark:border-dark-border rounded-xl hover:scale-105 active:scale-95 transition-all text-xs"
              title="Toggle theme"
            >
      ...
      {/* Tabs list */}
      <div className="flex gap-3 mb-6 border-b border-light-border dark:border-dark-border pb-2 overflow-x-auto">
      ...
      <div className="flex-1 bg-white dark:bg-dark-panel border border-light-border dark:border-dark-border rounded-3xl p-6 relative">
          ...
          <div className="space-y-4">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-light-border dark:border-dark-border text-light-secondary dark:text-dark-secondary uppercase font-bold text-[10px]">
                ...
                <tbody className="divide-y divide-light-border/40 dark:divide-dark-border/20 font-mono">
                  {transactions.filter(t => t.type === 'deposit').length === 0 ? (
                    <tr>
                      <td colSpan={6} className="text-center p-8 text-light-secondary dark:text-dark-secondary">No deposits found.</td>
                    </tr>
                  ) : (
                    transactions.filter(t => t.type === 'deposit').map((t) => (
                      <tr key={t.id} className="hover:bg-light-panel dark:hover:bg-dark-bg/25">
                        ...
                        <td className="p-3 text-[10px] text-light-secondary dark:text-dark-secondary">
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-light-border/40 dark:divide-dark-border/20 font-mono">
                  {transactions.filter(t => t.type === 'withdrawal').length === 0 ? (
                    <tr>
                      <td colSpan={6} className="text-center p-8 text-light-secondary dark:text-dark-secondary">No withdrawals found.</td>
                    </tr>
                  ) : (
                    transactions.filter(t => t.type === 'withdrawal').map((t) => (
                      <tr key={t.id} className="hover:bg-light-panel dark:hover:bg-dark-bg/25">
                        ...
                        <td className="p-3 text-[10px] text-light-secondary dark:text-dark-secondary font-sans">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-light-border/40 dark:divide-dark-border/20 font-mono">
                  {brokerApps.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="text-center p-8 text-light-secondary dark:text-dark-secondary">No applications found.</td>
                    </tr>
                  ) : (
                    brokerApps.map((app) => (
                      <tr key={app.id} className="hover:bg-light-panel dark:hover:bg-dark-bg/25">
                        ...
                        <td className="p-3 capitalize font-sans text-light-primary dark:text-dark-primary">{app.payment_options}</td>
                        <td className="p-3 text-[10px] text-light-secondary dark:text-dark-secondary font-sans">{app.payment_details}</td>
                        <td className="p-3">
                          <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold ${
        ...
        {activeTab === 'insights' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {/* Create form */}
            <form onSubmit={handleCreateInsight} className="space-y-4 bg-light-panel dark:bg-dark-bg/30 p-5 rounded-2xl border border-light-border dark:border-dark-border">
              ...
              <div>
                <label className="block text-[10px] font-bold text-light-secondary dark:text-dark-secondary uppercase mb-2">Insight Title</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-2 bg-white dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-xl text-xs focus:outline-none focus:border-[#2962FF] text-light-primary dark:text-dark-primary"
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-light-secondary dark:text-dark-secondary uppercase mb-2">Insight Content</label>
                <textarea
                  required
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                {insightsList.length === 0 ? (
                  <p className="text-center text-xs text-light-secondary dark:text-dark-secondary p-8">No reports published yet.</p>
                ) : (
                  insightsList.map((ins) => (
                    <div key={ins.id} className="p-4 bg-light-panel dark:bg-dark-bg/25 border border-light-border dark:border-dark-border/30 rounded-xl flex justify-between items-start gap-4">
                      <div>
                        ...
                        <p className="text-[10px] text-light-secondary dark:text-dark-secondary mt-1 line-clamp-2">{ins.content}</p>
                      </div>
                      <button
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-light-border/40 dark:divide-dark-border/20 font-mono">
                  {tradesList.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="text-center p-8 text-light-secondary dark:text-dark-secondary">No retail trades logged yet.</td>
                    </tr>
                  ) : (
                    tradesList.map((t) => {
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-light-panel dark:hover:bg-dark-bg/25">
                          <td className="p-3 text-light-primary dark:text-white font-sans">
                            ...
                            <span className="text-[10px] text-light-secondary dark:text-dark-secondary block font-mono">
                              {t.profile?.email || t.user_id}
                            </span>
                          </td>
                          <td className="p-3 text-light-primary dark:text-dark-primary uppercase font-bold">{t.asset}</td>
                          <td className="p-3">
                          ...
                          <td className="p-3 text-right text-light-secondary dark:text-dark-secondary">${Number(t.entry_price).toFixed(decimals)}</td>
                          <td className="p-3 text-right text-light-secondary dark:text-dark-secondary">${Number(t.exit_price || t.entry_price).toFixed(decimals)}</td>
                          <td className={`p-3 text-right font-bold ${
                            isProfit ? 'text-[#089981]' : isLoss ? 'text-[#F23645]' : 'text-light-secondary dark:text-dark-secondary'
                          }`}>
                          ...
                          <td className="p-3 text-light-secondary dark:text-dark-secondary font-sans text-[10px]">
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-light-border/40 dark:divide-dark-border/20 font-mono">
                  {reviewsList.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="text-center p-8 text-light-secondary dark:text-dark-secondary">No reviews submitted yet.</td>
                    </tr>
                  ) : (
                    reviewsList.map((rev) => (
                      <tr key={rev.id} className="hover:bg-light-panel dark:hover:bg-dark-bg/25">
                        <td className="p-3 font-sans text-light-primary dark:text-white">
                          ...
                          <div className="text-[10px] text-light-secondary dark:text-dark-secondary">{rev.role}</div>
                        </td>
                        <td className="p-3 font-sans text-amber-500 font-bold">
                          {Array.from({ length: rev.stars || 5 }).map(() => '★').join('')}
                        </td>
                        <td className="p-3 font-sans text-light-primary dark:text-dark-primary max-w-sm whitespace-pre-wrap leading-relaxed">
                          "{rev.quote}"
                        </td>
                        <td className="p-3">
                          <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold ${
        ...
        {activeTab === 'settings' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="p-5 bg-light-panel dark:bg-dark-bg/30 rounded-2xl border border-light-border dark:border-dark-border flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between mt-2 p-3 bg-white dark:bg-dark-bg rounded-xl border border-light-border dark:border-dark-border">
                <span className="text-xs font-bold text-light-primary dark:text-dark-primary">Enable Onboarding Signup Bonus ($10)</span>
                <button
                  onClick={() => handleToggleOnboarding(!onboardingEnabled)}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none ${
                    onboardingEnabled ? 'bg-[#089981]' : 'bg-light-panel dark:bg-dark-border'
                  }`}
                >
                  <span
            ...
            <form onSubmit={handleAwardBonus} className="p-5 bg-light-panel dark:bg-dark-bg/30 rounded-2xl border border-light-border dark:border-dark-border flex flex-col gap-4">
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-[9px] font-bold text-light-secondary dark:text-dark-secondary uppercase mb-1.5">User UUID</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-3 py-2 bg-white dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-xl text-xs focus:outline-none focus:border-[#2962FF] text-light-primary dark:text-dark-primary font-mono"
                  />
                </div>
                
                <div>
                  <label className="block text-[9px] font-bold text-light-secondary dark:text-dark-secondary uppercase mb-1.5">Amount (USD)</label>
                  <input
                    type="number"
                    required
                    min="1"
                ...
                <div>
                  <label className="block text-[9px] font-bold text-light-secondary dark:text-dark-secondary uppercase mb-1.5">Custom Alert Message</label>
                  <input
                    type="text"
