...
export default function Dashboard({ toggleTheme, theme }: DashboardProps) {
  ...
  }, [fetchActiveTrades, fetchTradeHistory, fetchInsights]);
            ...
            <div className="px-6 py-3 border-b border-light-border dark:border-[#2A2E39] flex items-center justify-between bg-white dark:bg-[#1E222D]">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-[#2962FF] hover:bg-opacity-95 text-white font-extrabold rounded-lg text-xs transition-all"
                >
                  Switch Asset
                </button>
          ...
          <div className="flex-1 overflow-hidden">
            ...
            {activeMobileTab === 'chart' && (
              <div className="flex flex-col h-full">
                {/* 60% Top: Chart */}
                <div className="h-[55%] border-b border-light-border dark:border-dark-border/40 relative">
                  <Chart symbol={activeAsset} theme={theme} />
                </div>

                {/* 40% Bottom: Exec Ticket */}
                <div className="flex-1 p-4 overflow-y-auto bg-light-panel dark:bg-[#151924]">
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      <label className="block text-[9px] font-bold text-light-secondary dark:text-dark-secondary uppercase mb-1">Investment ($)</label>
                      <input
                        type="number"
                        value={quantity}
                        onChange={(e) => setQuantity(e.target.value)}
                        className="w-full px-3 py-1.5 bg-white dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-lg text-xs"
                      />
                    </div>
                    <div>
                      <label className="block text-[9px] font-bold text-light-secondary dark:text-dark-secondary uppercase mb-1">
                        Duration ({isSynthetic ? 's' : 'h'})
                      </label>
                      <input
                        type="number"
                        value={duration}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onClick={() => handleOpenTrade('buy')}
                      disabled={wsState !== 'connected'}
                      className={`py-3 bg-bullish text-white font-extrabold rounded-xl text-xs uppercase ${wsState !== 'connected' ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
                    >
                    ...
                    <button
                      onClick={() => handleOpenTrade('sell')}
                      disabled={wsState !== 'connected'}
                      className={`py-3 bg-bearish text-white font-extrabold rounded-xl text-xs uppercase ${wsState !== 'connected' ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
                    >
            ...
            {activeMobileTab === 'trades' && (
              <div className="flex flex-col h-full bg-[#111112] text-[#D1D4DC] p-4 font-sans">
                {/* Glassmorphic Summary Top */}
                <div className="bg-[#1E222D]/60 backdrop-blur-md border border-[#2A2E39] rounded-2xl p-4 flex flex-col gap-3.5 shadow-xl mb-4 relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-24 h-24 bg-[#2962FF]/5 rounded-full blur-2xl pointer-events-none" />

                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col">
                      <span className="text-[9px] text-[#8A91A5] font-bold uppercase tracking-wider">Balance</span>
                      <span className="font-mono font-black text-sm text-[#D1D4DC]">
                        ${activeBalance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] text-[#8A91A5] font-bold uppercase tracking-wider">Equity</span>
                      <span className={`font-mono font-black text-sm ${floatingPnl >= 0 ? 'text-[#089981]' : 'text-[#F23645]'}`}>
                        ${equity.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4 border-t border-[#2A2E39]/50 pt-3">
                    <div className="flex flex-col">
                      <span className="text-[9px] text-[#8A91A5] font-bold uppercase tracking-wider">Free Margin</span>
                      <span className="font-mono font-black text-sm text-[#D1D4DC]">
                        ${freeMargin.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] text-[#8A91A5] font-bold uppercase tracking-wider">Free Margin %</span>
                      <span className="font-mono font-black text-sm text-[#089981]">
                ...
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-black text-xs uppercase tracking-wider text-[#D1D4DC]">ACTIVE POSITIONS</h3>
                  {activeTrades.length > 0 && (
                    <button
                      onClick={async () => {
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-1 pb-16">
                  {activeTrades.map((t) => (
                    <div key={t.id} className="p-3 bg-[#1E222D] rounded-xl border border-[#2A2E39] flex justify-between items-center shadow-md">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-black text-xs text-[#D1D4DC]">{t.asset}</span>
                        ...
                        <span className="text-[10px] text-[#8A91A5] mt-1 block">Vol: {t.quantity.toFixed(2)} | Entry: {t.entry_price.toFixed(t.asset.includes('JPY') ? 3 : 5)}</span>
                      </div>
                      <div className="flex items-center gap-3">
                  ...
                  {activeTrades.length === 0 && (
                    <div className="h-48 flex flex-col items-center justify-center text-center">
                      <p className="text-xs text-[#8A91A5]">No active positions.</p>
                      <p className="text-[10px] text-[#8A91A5]/60 mt-1">Open the Chart tab to place a trade.</p>
                    </div>
                  )}
                </div>
              </div>
