...
export default function Chart({ symbol, theme }: ChartProps) {
  ...
  useEffect(() => {
    ...
    const gridRows = 6;
    const rowHeight = chartHeight / gridRows;
    ...
    // Draw vertical grid lines and date/time labels
    const timeBoundary = timeframe === 'M1' ? 3600 : 21600; // 1 hour or 6 hours
    let lastTimeBlock = -1;
    ...
    visibleCandles.forEach((candle, index) => {
      ...
      const timeBlock = Math.floor(candle.time / timeBoundary);
      if (lastTimeBlock === -1) {
        lastTimeBlock = timeBlock;
      } else if (timeBlock !== lastTimeBlock) {
        lastTimeBlock = timeBlock;
        ...
        // Draw time label at the bottom margin
        ...
        ctx.fillStyle = activeColors.text;
        ctx.fillText(label, x - 20, chartHeight + 14);
      }
    });
