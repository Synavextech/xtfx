...
// Tick loop: runs every 500ms to generate live prices, update candles, and evaluate open positions
    ...
    for (const asset of ASSETS) {
      ...
      for (const trade of tradesForAsset) {
        ...
        if (trade.stop_loss !== null) {
          if ((trade.type === 'buy' && newPrice <= trade.stop_loss) || 
            ...
            const investment = (trade.quantity * trade.entry_price) / 100;
            const cappedLossPnl = Math.max(-investment, lossPnl);
        ...
        if (trade.take_profit !== null) {
          if ((trade.type === 'buy' && newPrice >= trade.take_profit) || 
